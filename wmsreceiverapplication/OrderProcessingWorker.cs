        ...
        public OrderProcessingWorker(ILogger<OrderProcessingWorker> logger)
        {
            ...
            while (retries > 0)
            {
                try
                {
                    ...
                    _logger.LogInformation("Connected to RabbitMQ for OrderProcessing queue");
                    break;
                }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                if (result != null)
                {
                    ...
                    var customer = JsonSerializer.Deserialize<PurchaseOrder>(message);
                    ...
                    try
                    {
                        var response = await _httpClient.PostAsync("http://wmsapi:80/api/PurchaseOrder/orders", content, stoppingToken);
